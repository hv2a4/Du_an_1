...
public class LoaiSanPham extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        txtMaLoai.setFont(new java.awt.Font("Arial", 0, 13)); // NOI18N
        ...
        txtTenLoai.setFont(new java.awt.Font("Arial", 0, 13)); // NOI18N
    ...
    public boolean trungMa(String maLoai) {
        List<LoaiSanPhamObject> list = lspdao.selectAll();
        for (LoaiSanPhamObject loaiSanPhamObject : list) {
            if (loaiSanPhamObject.getMaLoaiSanPham().equals(maLoai)) {
                return true;
            }
        }
        return false;
    ...
    private static boolean isMaKhachHangInvalid(String maKhachHang) {
        ...
        return maKhachHang != null && !maKhachHang.matches("LSP\\d{3}");
    }

    public String hashError() {
        String maSanPham = txtMaLoai.getText();
        String tenSanPham = txtTenLoai.getText();
        if (maSanPham.isEmpty() || tenSanPham.isEmpty()) {
            return "Vui lòng điền thông tin quan trọng!";
        }
        if (trungMa(maSanPham)) {
            return "Mã loại sản phẩm này đã tồn tại!";
        }
        if (isMaKhachHangInvalid(maSanPham)) {
    ...
    public void update() {
        ...
        if (isMaKhachHangInvalid(maLoai)) {
            MsgBox.warning(this, "Mã loại định dạng không hợp lệ!");
            return;
        }
        int row = tblLoaiSanPham.getSelectedRow();
        if (row < 0) {
            MsgBox.alert(this, "Vui lòng chọn hoặc nhập dữ liệu để chỉnh sửa thông tin!");
            return;
        }
        LoaiSanPhamObject lsp = getForm();
        try {
